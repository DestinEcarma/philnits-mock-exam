...
export default function Results({ quizData, userAnswers, onRestart, timeRemaining, timeTaken }: ResultsProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      {isTimeOut ? (
        <p className="text-xl text-red-600">Time's up!</p>
      ) : (
        <p className="text-xl text-green-600">Quiz completed!</p>
      )}
      <p className="text-xl">
      ...
      {quizData.map((question, index) => (
        <div key={index} className={`border p-4 rounded-md ${userAnswers[index] === question.correctAnswer ? 'bg-green-50' : 'bg-red-50'}`}>
          ...
          {userAnswers[index] !== undefined ? (
              ...
              {question.solution && (
                <Accordion type="single" collapsible className="w-full">
                  <AccordionItem value="solution">
                    ...
                    <AccordionContent>
                      <div className="relative w-[10rem] h-[100rem]">
                        <Image
            ...
            <p className="mt-2 text-red-600">Not answered</p>
          )}
        </div>
      ))}
